if (typeof(COMMON_JS2) == 'undefined') {
    ...
    var clSVG = function(type, w, h, wrap, add_class) {
        ...
        var icon = '';
        switch (type) {
            case 'ctrl_card_caret':
                icon = `<svg viewBox="0 0 ${w} ${h}" width="${w}" height="${h}"><polygon points="4 0 2.94 1.06 8.88 7 2.94 12.94 4 14 11 7 "></polygon></svg><svg class="open" viewBox="0 0 ${w} ${h}" width="${w}" height="${h}"><polygon points="12.94 2.94 7 8.88 1.06 2.94 0 4 7 11 14 4 "></polygon></svg>`;
                break;
            case 'ctrl_dropdown_caret':
                icon = `<svg viewBox="0 0 ${w} ${h}" width="${w}" height="${h}"><polygon points="12.94 2.94 7 8.88 1.06 2.94 0 4 7 11 14 4 "></polygon></svg>`;
                break;
            ...
            case 'caret_down':
                icon = `<svg viewBox="0 0 10 10" width="${w}" height="${h}"><path d="m1 4.01 4 4 4-4H1z"/></svg>`;
                break;

            case 'caret_right':
                icon = `<svg viewBox="0 0 10 10" width="${w}" height="${h}"><path d="m4 9.01 4-4-4-4v8z"/></svg>`;
                break;
            ...
            case 'weight':
                icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="${w}" height="${h}"><path d="M7 5h4.45c2.76 0 4.86.77 4.86 3.2 0 1.19-.69 2.42-1.83 2.81v.09c1.45.33 2.52 1.32 2.52 3.07 0 2.62-2.25 3.83-5.21 3.83H7V5zm4.25 5.29c1.71 0 2.45-.67 2.45-1.7 0-1.14-.8-1.6-2.41-1.6h-1.6v3.3h1.56zm.31 5.71c1.83 0 2.83-.63 2.83-1.98 0-1.27-.98-1.79-2.83-1.79H9.69V16h1.87z"></path></svg>`;
                break;
            case 'italic':
                icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="${w}" height="${h}"><path d="m7.98 18 .15-.76h.24c.41 0 .78-.08 1.11-.23.33-.15.55-.51.67-1.08L12.04 7c.05-.19.07-.36.07-.51 0-.32-.12-.52-.36-.6a3.06 3.06 0 0 0-.95-.13h-.24l.17-.76h5.3l-.16.76h-.24c-.41 0-.78.08-1.1.23-.32.15-.54.51-.66 1.08L11.96 16c-.05.19-.07.36-.07.51 0 .32.12.52.36.6.24.08.56.13.95.13h.24l-.17.76H7.98z"></path></svg>`;
                break;
            case 'linethrough':
                icon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="${w}" height="${h}"><path d="M21 11.94h-6.57a6.6 6.6 0 0 0-.9-.52l-1.67-.79c-.99-.44-1.96-.88-1.96-2.03 0-1.08.81-1.74 2.05-1.74 1.07 0 1.91.46 2.67 1.21l1.01-1.36C14.7 5.66 13.36 5 11.94 5 9.63 5 7.95 6.6 7.95 8.72c0 1.63.9 2.61 1.88 3.22H3V13h8.96l.15.07c1.12.55 1.95.94 1.95 2.17 0 1.14-.83 1.91-2.29 1.91-1.17 0-2.36-.62-3.23-1.6l-1.12 1.47A5.7 5.7 0 0 0 11.73 19c2.66 0 4.27-1.76 4.27-3.93 0-.86-.21-1.53-.55-2.07H21v-1.06z"></path></svg>`;
                break;
            ...
            case 'file-image-none':
                icon = `<svg viewBox="0 0 26 26" width="${w}" height="${h}"><path d="m24.94 0-3 3H3v18.94l-3 3L1.06 26 26 1.06 24.94 0ZM4.5 4.5h15.94l-7.47 7.47L11 10l-6.5 6.5v-12ZM21.5 15.5 19 18l-3.92-3.92-7.42 7.42-1.5 1.5H23V6.16l-1.5 1.5v7.84z"/><circle cx="7.5" cy="7.5" r="1.5"/></svg>`;
                break;
            ...
            case 'block-sync':
                ...
                break;
            case 'block-calendar':
                icon = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 38 38" width="38" height="38"><path d="M29 10h-4V8c0-.55-.45-1-1-1s-1 .45-1 1v2h-8V8c0-.55-.45-1-1-1s-1 .45-1 1v2H9c-1.65 0-3 1.35-3 3v14c0 1.65 1.35 3 3 3h20c1.65 0 3-1.35 3-3V13c0-1.65-1.35-3-3-3ZM9 12h4v2c0 .55.45 1 1 1s1-.45 1-1v-2h8v2c0 .55.45 1 1 1s1-.45 1-1v-2h4c.55 0 1 .45 1 1v4H8v-4c0-.55.45-1 1-1Zm20 16H9c-.55 0-1-.45-1-1v-8h22v8c0 .55-.45 1-1 1Z"/></svg>';
                break;
            ...
            case 'radio_thumbsvg_sort_n':
                icon = `<svg viewBox="0 0 22 22" width="${w}" height="${h}"><rect x="1" y="1" width="9" height="9"/><rect x="12" y="1" width="9" height="9"/><rect x="12" y="12" width="9" height="9"/><rect x="1" y="12" width="9" height="9"/></svg>`;
                break;
            case 'radio_thumbsvg_sort_m':
                icon = `<svg viewBox="0 0 22 22" width="${w}" height="${h}"><rect x="1" width="9" height="7"/><rect x="12" y="15" width="9" height="7"/><rect x="12" width="9" height="13"/><rect x="1" y="9" width="9" height="13"/></svg>`;
                break;
            ...
            case 'field_hit':
            case 'field_hit_default':
                icon = `<svg viewBox="0 0 16 16" width="${w}" height="${h}"><path d="M10.4 6.9c.4 1.3-.4 2.7-1.8 3.1-1.3.4-2.7-.4-3.1-1.8v-.5c0-.2.2-.2.3-.2h.5c.9 0 1.7-.7 1.7-1.7v-.5c0-.1 0-.3.2-.3 1.1 0 2 .8 2.3 1.9ZM8 4.3c-2.7 0-4.1 1.8-4.9 3.4v.6C4 9.9 5.3 11.7 8 11.7s4.1-1.8 4.9-3.4v-.6C12 6.1 10.7 4.3 8 4.3m0-1c3.3 0 4.9 2.3 5.8 4 .3.5.3 1 0 1.5-.9 1.6-2.5 4-5.8 4s-4.9-2.3-5.8-4c-.3-.5-.3-1 0-1.5.9-1.6 2.5-4 5.8-4Z"/></svg>`;
                break;

            case 'field_comment':
            case 'field_comment_default':
                icon = `<svg viewBox="0 0 16 16" width="${w}" height="${h}"><path d="M9.6 3h-3C4.3 3 2.2 4.7 2 7c-.1 1.6.6 3 1.8 3.8 0 .5-.2 1-.5 1.6-.1.3.1.6.4.6h.2c.7-.3 1.6-.7 2.4-1.3h3c2.3 0 4.3-1.6 4.6-3.9.2-2.6-1.8-4.7-4.4-4.7Zm-.2 7.6H6.1l-.3.2c-.3.3-.7.5-1 .6v-1.1l-.4-.4c-1-.7-1.5-1.8-1.4-3 .1-1.7 1.7-3.1 3.6-3.1h3c1 0 1.9.4 2.5 1.1.6.7.9 1.6.8 2.5-.2 1.7-1.7 3-3.6 3Z"/></svg>`;
                break;

            case 'field_like':
            case 'field_like_default':
                icon = `
                    <svg class="active" viewBox="0 0 16 16" width="${w}" height="${h}"><path d="M12.6 3.2c-.8-.6-1.7-.8-2.6-.7-.7.1-1.4.4-2 1-.7-.7-1.6-1-2.5-1-.9 0-1.9.4-2.5 1.1C1.7 5 1.7 7.3 3 8.7l4.6 4.7s.2.1.3.1.3 0 .3-.1l4.6-4.6c.1-.2.2-.3.3-.4 1.2-1.6.9-3.9-.7-5Z"/></svg>
                    <svg class="inactive" viewBox="0 0 16 16" width="${w}" height="${h}"><path d="M12.6 3.2c-.6-.5-1.4-.7-2.1-.7H10c-.7.1-1.4.4-2 1-.5-.5-1.2-.9-2-1h-.5c-.8 0-1.5.3-2.1.7-1.6 1.2-1.9 3.5-.7 5 .1.1.2.3.3.4s4.6 4.7 4.6 4.7.2.1.3.1c.1 0 .3 0 .3-.1l4.6-4.6c.1-.2.2-.3.3-.4 1.2-1.6.9-3.9-.7-5Zm-.1 4.5c0 .1-.2.2-.2.3l-4.2 4.3L3.9 8s-.2-.2-.2-.3c-.9-1.2-.6-2.8.5-3.7.4-.3 1-.5 1.5-.5h.4c.5 0 1 .3 1.4.7l.7.7.7-.7c.4-.4.9-.6 1.4-.7h.4c.6 0 1.1.2 1.5.5 1.1.9 1.4 2.5.5 3.7Z"/></svg>
                `;
                break;

            case 'field_guestbook_more_default':
                icon = `
                    <svg viewBox="0 0 14 14" width="14" height="14"><g><path d="M6 2h2v2H6z"></path><path d="M6 6h2v2H6z"></path><path d="M6 10h2v2H6z"></path></g></svg>
                `;
                break;

            case 'field_faq_toggle_arrow':
                icon = `<svg class="arrow" viewBox="0 0 18 18" width="18" height="18"><polygon points="17.29 4 9 12.29 0.71 4 0 4.71 9 13.71 18 4.71 "/></svg>`;
                break;

            case 'field_faq_toggle_arrow2':
                icon = `<svg class="arrow" viewBox="0 0 16 16" width="16" height="16"><path d="M8 11.5 5.12 7.95 2 4.5l6 .1 6-.1-3.12 3.45z"/></svg>`;
                break;

            case 'field_faq_toggle_plusminus':
                icon = `
                    <svg class="close" viewBox="0 0 16 16" width="16" height="16"><g><path d="M7.5 13.5v-5h-5v-1h5v-5h1v5h5v1h-5v5z"/><path d="M8 3v5h5-5v5V3m1-1H7v5H2v2h5v5h2V9h5V7H9V2z"/></g></svg>
                    <svg class="open" viewBox="0 0 16 16" width="16" height="16"><g><path d="M2.5 7.5h11v1h-11z"/><path d="M14 7H2v2h12V7z"/></g></svg>
                `;
                break;

            case 'comment_default':
                icon = '<svg viewBox="0 0 14 14" width="' + w + '" height="' + h + '"><path d="M7 1C3.13 1 0 3.24 0 6c0 1.66 1.14 3.13 2.89 4.04C2.71 11 2.23 12.38 1 13c0 0 3.09 0 5.19-2.04.27.03.54.04.81.04 3.87 0 7-2.24 7-5s-3.13-5-7-5zm0 9c-.25 0-.49-.01-.73-.03l-.45-.04-.32.31c-.6.58-1.31.97-1.98 1.23.17-.43.28-.86.35-1.25l.14-.73-.66-.34C1.88 8.39 1 7.21 1 6c0-2.17 2.75-4 6-4s6 1.83 6 4-2.75 4-6 4z"/></svg>';
                break;

            case 'like_default':
                icon = '<svg viewBox="0 0 28 28" width="' + w + '" height="' + h + '"><path d="m22.76 14.7-8.74 8.97-8.75-8.97m0 0c-2.31-2.32-2.37-6.14-.13-8.53 2.24-2.4 5.93-2.46 8.24-.14.23.23.44.48.63.75 1.93-2.67 5.59-3.2 8.16-1.2s3.09 5.79 1.16 8.45c-.18.24-.37.47-.58.68"></path><path d="M14.01 24.57c-.24 0-.48-.1-.64-.27l-8.74-8.97c-2.64-2.66-2.71-7.04-.15-9.78a6.576 6.576 0 0 1 4.74-2.12c1.79-.01 3.5.67 4.8 1.96a6.576 6.576 0 0 1 3.74-1.86c1.77-.25 3.54.22 4.97 1.34 2.95 2.29 3.55 6.64 1.33 9.69-.2.28-.42.54-.66.78l-.06.06-8.68 8.9c-.17.17-.4.27-.65.27zm-8.1-10.51c0 .01.01.01 0 0l8.1 8.32 8.1-8.31.06-.06c.15-.16.3-.33.43-.52 1.65-2.27 1.21-5.51-.98-7.22a4.723 4.723 0 0 0-3.61-.97c-1.31.2-2.47.91-3.27 2a.91.91 0 0 1-.73.37c-.29 0-.56-.14-.73-.38-.16-.22-.34-.44-.54-.64a4.78 4.78 0 0 0-3.49-1.43A4.83 4.83 0 0 0 5.8 6.78c-1.91 2.04-1.86 5.31.11 7.28z"></path></svg>';
                break;
            ...
            case 'move_link': 
                icon = `<svg xmlns="http://www.w3.org/2000/svg"  width="${w}" height="${h}" viewBox="0 0 12 12"><path d="M10 11H1V2h4V1H0v11h11V7h-1v4zM7 0v1h4v4h1V0H7z"/><path d="M11 1 5.5 6.5" style="fill:none;stroke:#000;stroke-miterlimit:10"/></svg>`;
                break;
    ...
    var show404 = function() {
        ...
        if($('.error_404.new_design').length == 0) {
            $('.dsgn-body').append(error_style);
            $('div.element.el-footer').before(error_html); //404 error
        } else {
            $('.error_404').show();
        }
    }
