...
$(function() {
	...
	$.psearch = {
		...
		showSiteProjectSEARCH: function(view_page) {
			...
			var active_nav = $('.el-menu header');
			if(window.innerWidth <= 768) active_nav = active_nav.is('.cl-nav') ? $('.clnav-sidemobile') : $('#mobile-nav');

			var psearchWarp = active_nav.find('.psearch-overlay');
	...
	$.umember = {		
		...
		makeJoinCollectionsTypeVal: function(oval_i,oval_type,oval_v,org_field,optional_class,check_page,val) {
			...
			switch (oval_type) {
				...
				case 'fupload':
						// todo 데이터 잇없 체크
						str += $.umember.fileHtmlRender(org_field, {
										val : val,
										type : "type1",
									})
					break;
					...
					$.each(data['collections'], function(field,o) {
						...
						if(o['used']) {
							var org_field = (field == 'addr') ? field + '1' : field,
								...
								jcollection_val_str = '',
								fupload_label = '',
 								etc_val_type = ['memo','radio','check','select','fupload'];
							...
							if(field.indexOf('etc')>-1 && etc_val_type.includes(o['type'])) {
								var o_val = o['val'],
									o_val_arr = ((o_val.indexOf('`') > -1) && (o['type']=='radio' || o['type']=='check' || o['type']=='select'|| o['type']=='fupload')) ? o_val.split('`') : [o_val],
									description_class = (o['description_used']=='true' && (o['type']=='memo')) ? 'use_descr' : '';

								add_input = '\
								<div class="cl-s-join-collections-etcwrap cl-s-form-wrap '+(o['type']=='fupload' ? 'nodata' : '') +' '+ optional_class + '" data-jcoltype="'+o['type']+'">';
								add_input += '\
									<div class="cl-s-etc">\
								...
								// /*라벨 앞에 <input type="text" class="hide" value=""> 이거 있었음*/
								// if(o['type']=='fupload') { // JH
								// 	add_input += $.umember.fileHtmlRender('join', {
								// 	orgfield : org_field
								// 	})
								...
								if(mode == 'shopping_social' && !o['used']) add_input = '';
		...
		makeMypageConditionsStr: function(sid,data,mode,cert) {
			...
			var collections = '',
				make_pass = ['sid', 'no', 'id', 'image', 'regExp', 'cert', 'cert_date', 'cert_adult', 'cert_adult_date','sm_addr'];
					...
					if($.inArray(field,make_pass) == -1) {
						var name = (o['name']) ? o['name'] : $.lang[LANG]['manager.list.info.'+field],
							...
							jcollection_val_str = '';
							 ...
							 var filecount_data = o['type'] === 'fupload' && o['value'] ? 'isdata' : o['type'] === 'fupload' ? 'nodata' : '';
						...
						if(field.indexOf('etc')>-1 && etc_val_type.includes(o['type'])) {
							...
							add_input = '\
							<div class="cl-s-join-collections-etcwrap cl-s-form-wrap '+filecount_data+' '+ optional_class + '" data-jcoltype="'+o['type']+'">';
						...
						if(field == 'birth') {
							...
							if(  mode && mode === 'business'){
								...
								if (data['sm_addr'] && data['sm_addr']['is_default'] === 0) {
									sm_nodata = 'emptyFlag';
									btn_text = '추가하기';
								} else{
									const sm = data?.sm_addr || {};

									sm_name    = sm.name?.trim?.() || '';
									sm_tel     = sm.tel?.trim?.() || '';
									
									addrText   = sm.value?.[0]?.trim?.() || '';
									sm_addr0   = addrText ? '[' + addrText + ']' : '';
									sm_addr0_empty = sm_addr0 ? '' : 'hide';

									sm_addr1   = sm.value?.[1]?.trim?.() || '';
									sm_addr2   = sm.value?.[2]?.trim?.() || '';
									aliasText = sm.alias?.trim?.() || '';  // 먼저 alias 문자열 뽑기
									sm_isalias = aliasText ? '(' + aliasText + ')' : '';  // 괄호 처리
									sm_isalias_empty = aliasText ? '' : 'hide';  // 클래스 처리
								...
								var valueCheckEl = "";
								console.log(sm_nodata);
								(valueCheckEl = sm_nodata ? '<input type="text" class="cl-s-form-control hide" required="required" value="">' : '' );
								add_input = '\
								<div class="cl-s-form-wrap sm-parent '+sm_nodata+'">\
									<div class="cl-s-form-wrap '+(sm_nodata ? optional_class : "") +'">\
										<div class="cl-s-form-group">\
											<div class="cl-s-form-control flexbox">\
											'+valueCheckEl+'\
												<label for="input" class="cl-s-control-label">배송지 정보<span class="input-star '+ (optional_class ? '' : 'hide') +'">*</span></label>\
												<div class="cl-s-form-wrap btn-load-addresses mypage" data-action="getList">\
													<div class="cl-s-form-group EditBtn">\
													<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16"><path d="m15.29 7-1.31 1.31c.16-3-1.96-5.71-5-6.22-1.91-.32-3.86.3-5.23 1.67l.71.71a5.003 5.003 0 0 1 7.07 0 4.989 4.989 0 0 1 1.45 3.81L11.7 7l-.71.71 2.5 2.5 2.5-2.5-.71-.71ZM4.46 11.53a4.989 4.989 0 0 1-1.45-3.81L4.29 9 5 8.29l-2.5-2.5L0 8.29.71 9l1.31-1.31c-.16 3 1.96 5.71 5 6.22 1.91.32 3.86-.3 5.23-1.67l-.71-.71a5.003 5.003 0 0 1-7.07 0Z"/></svg><span>'+btn_text+'</span>\
													</div>\
													<div class="cl-s-form-group ">\
													<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 12" width="12" height="12"><path d="M6.5 12h-1V6.5H0v-1h5.5V0h1v5.5H12v1H6.5V12Z"/></svg><span>'+btn_text+'</span>\
													</div>\
												</div>\
											</div>\
											<div class="cl-s-form-control"><div class="sm-name">'+sm_name+'</div><div class="sm-alias '+sm_isalias_empty+'">'+sm_isalias+'</div><div class="sm-default-addr"><span>기본배송지</span></div></div>\
											\
											<div class="cl-s-form-control"><div class="sm-addr1"><span class ="'+sm_addr0_empty+'">'+sm_addr0+'</span><span>'+sm_addr1+' '+sm_addr2+'</span></div></div>\
											<div class="cl-s-form-control"><div class="sm-tel">'+sm_tel+'</div></div>\
											<div class="cl-s-form-control noaddress" style="text-align:center; padding:20px; color:#999;">등록된 배송지가 없습니다.</div>\
										</div>\
									</div>\
								</div>\
								';		
		...
		fileurlcreate : function(cl_val) {
			// 단일 파일 URL 생성 함수
			// cl_val을 받아서 다운로드 링크와 파일 정보를 반환
			var result = {
				download_link: '',
				file_links: '',
				file_data: '',
			...
			if(cl_val && cl_val.trim() !== '') {
				// 단일 파일 처리 - 첫 번째 파일만 사용
				var file_data = cl_val.split(',')[0];
				result.file_data = file_data;
				
				if(file_data && file_data.trim() !== '') {
					var explode_v = file_data.split('||');
					// console.log('fileurlcreate explode_v:', explode_v); // 디버깅용
					
					// 현재 데이터 구조에 맞게 수정
					// explode_v[0]: 파일명, explode_v[1]: 파일 크기, explode_v[2]: URL
					var link_url = explode_v[2] || '';
					var link_url_arr = link_url.split('/');
					var true_file_name = link_url_arr[link_url_arr.length - 1];
					
					if(explode_v[0]) {
						// 파일 크기 계산 (explode_v[1]에서 크기 추출)
						var flink_s = (explode_v[1]) ? parseFloat(explode_v[1].replace(/[^0-9\.]/g, '')) : 0;
						var flink_unit = (explode_v[1] && explode_v[1].indexOf('MB') > -1) ? 'MB' : 'KB';
						var flink_size = (flink_unit === 'MB') ? flink_s : flink_s;
						// view.html의 round($flink_size,2)와 동일하게 소수점 2자리까지 표시
						var flink_str = (explode_v[1]) ? parseFloat(flink_size.toFixed(2)) + flink_unit : '';
						
						var current_sid = (typeof property !== 'undefined' && property.SID) ? property.SID : (typeof SID !== 'undefined' ? SID : '');
						var download_link = '<a href="/down/flink/1/sid/' + current_sid + '/name/' + encodeURIComponent(explode_v[0]) + '/s/' + true_file_name + '" class="file-download-btn" target="_blank">';
						
						result.download_link = download_link;
						result.file_links = download_link;
						result.file_info = {
							name: explode_v[0],
							size: explode_v[1] || '',
							url: link_url,
							size_display: flink_str
						};
					}
				}
			}
		...
		fileHtmlRender : function(orgfield,option){// 그리기 담당 메서드
 			// 온 값에따라 그리는거 달라지기 
			option = $.extend({
				val : "",
				type : "",
			...
			// option.val에서 파일 정보 파싱 (파일명||파일크기||파일경로 형태)
			var fileName = '';
			var fileSize = '';
			var filePath = '';
			var fileSizeDisplay = '';
			...
			// option.val이 있고 파일 정보 형식이 맞는지 확인
			if (option.val && option.val.indexOf('||') > -1) {
				var fileData = option.val.split('||');
				if (fileData.length >= 3) {
					fileName = fileData[0];
					fileSize = fileData[1];
					filePath = fileData[2];
					filePaths = (filePath && filePath.indexOf('/') > -1 ? [filePath.split('/').pop()] : []);
					// 파일명과 경로가 모두 있는 경우에만 유효한 파일로 판단
					if (fileName && filePath) {
						hasValidFile = true;
						
						// 파일 크기 단위 처리
						if (fileSize.indexOf('KB') > -1) {
							fileSize = parseFloat(fileSize.replace('KB', ''));
							fileSizeUnit = 'KB';
						...
						// 파일 크기 표시 형식 조정
						if (fileSizeUnit === 'KB' && fileSize > 1024) {
							fileSizeDisplay = (fileSize / 1024).toFixed(2) + 'MB';
						} else {
							fileSizeDisplay = fileSize.toFixed(2) + fileSizeUnit;
						}
					}
				}
			}
			...
			if(option.type == 'type1') {
				if (hasValidFile) {
					...
					temphtml +=	'\
					<div class="cl-s-form-group">\
						<input type="text" class="cl-s-form-control valueCheck"  value="' + option.val + '" required="required" name="'+orgfield+'" id="input-' + orgfield + '">\
						<div class="cl-s-form-control-register">\
							<p class="file-title text hide">첨부된 파일이 없습니다.</p>\
							<label r-fupload-type="file.upload" r-fupload-require="" class="hide"><input type="file" class="hide" ><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16"><path d="M1 15v-3.5h1V14h12v-2.5h1V15H1Zm6.5-2.79v-9.3L4.35 6.06l-.71-.71L8 1l4.35 4.35-.71.71-3.15-3.15v9.29h-1Z"/></svg>' + $.lang[LANG]['register.fileUpload.btn.text'] + '</label>\
							<div class="file-row display-flex align-items-center">\
								<div class="r-flex-left">\
									<div class="file-thumb"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16"><path d="M4 16c-1.66 0-3-1.34-3-3V6l6-6h5c1.66 0 3 1.34 3 3v10c0 1.66-1.34 3-3 3H4Zm-2-3c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V3c0-1.1-.9-2-2-2H8v4c0 1.1-.9 2-2 2H2v6Zm.41-7H6c.55 0 1-.45 1-1V1.41L2.41 6ZM4 13v-1h8v1H4Zm0-3V9h8v1H4Z"/></svg></div>\
									<div>\
										<p class="file-name">'+fileName+'</p>\
									</div>\
								</div>\
								<div class="r-flex-right">\
									<p class="file-size">'+fileSizeDisplay+'</p>\
									<span class="form-file-delete" data-src="'+filePaths+'">\
					...
					temphtml +=	'\
					<div class="cl-s-form-group">\
						<input type="text" class="cl-s-form-control valueCheck"  value=""  required="required" name="'+orgfield+'" id="input-' + orgfield + '">\
						<div class="cl-s-form-control-register"  >\
							<p class="file-title hide text">첨부된 파일이 없습니다.</p><label r-fupload-type="file.upload" r-fupload-require="" class=""><input type="file" class="hide" ><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16"><path d="M1 15v-3.5h1V14h12v-2.5h1V15H1Zm6.5-2.79v-9.3L4.35 6.06l-.71-.71L8 1l4.35 4.35-.71.71-3.15-3.15v9.29h-1Z"/></svg>' + $.lang[LANG]['register.fileUpload.btn.text'] + '</label></div>\
						</div>\
					</div>\
					';
				}
			} else if(option.type == 'type2') {
				if(!option.val){
					temphtml += '\
					<div class="type2 cl-common-form-wrap nonplace-holder  '+ (option.val ? 'isdata' : 'nodata') +'" data-jcoltype="fupload" id="member-'+ orgfield + '">\
						<div class="cl-common-form-group">\
									<div class="cl-s-form-group">\
										<input type="text" class="member-info-input form-control cl-s-form-control valueCheck"  value=""  required="required" name="member-'+orgfield+'" id="member-' + orgfield + '">\
										<div class="cl-s-form-control-register"  >\
										<label r-fupload-type="file.upload" r-fupload-require="" class=""><input type="file" class="hide" ><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16"><path d="M1 15v-3.5h1V14h12v-2.5h1V15H1Zm6.5-2.79v-9.3L4.35 6.06l-.71-.71L8 1l4.35 4.35-.71.71-3.15-3.15v9.29h-1Z"/></svg>' + $.lang[LANG]['register.fileUpload.btn.text'] + '</label></div>\
										</div>\
									</div>\
					</div>';					
				} else {
					...
					<div class="type2 cl-common-form-wrap nonplace-holder  '+ (option.val ? 'isdata' : 'nodata') +'" data-jcoltype="fupload" id="member-'+ orgfield + '">\
						<div class="cl-common-form-group">\
									<div class="cl-s-form-group">\
										<input type="text" class="member-info-input form-control cl-s-form-control valueCheck"  value="'+option.val+'"  required="required" name="member-'+orgfield+'" id="member-' + orgfield + '">\
										<div class="cl-s-form-control-register"  >\
																<div class="file-row file-info-layout">\
																	<div class="file-left-area">\
																		<p class="file-name">'+fileName+'</p>\
																	</div>\
																	<div class="file-right-area">\
																		<span class="file-size">'+fileSizeDisplay+'</span>\
																		'+$.umember.fileurlcreate(option.val).download_link+'\
																			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">\
																				<path d="M5 19v-3.5h1V18h12v-2.5h1V19H5Zm2.65-7.15.71-.71 3.15 3.15V5h1v9.29l3.15-3.15.71.71-4.35 4.35-4.35-4.35Z"/>\
																			</svg>\
																		</a>\
																		<span class="form-file-delete" data-src="'+filePaths+'">\
																			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">\
																				<path d="M19 8h-3V6H8.02v2H5v1h1v9c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V9h1V8ZM9.02 7H15v1H9.02V7ZM17 18c0 .55-.45 1-1 1H8c-.55 0-1-.45-1-1V9h10v9Zm-7-7h1v6h-1v-6Zm3 0h1v6h-1v-6Z"/>\
																			</svg>\
																		</span>\
																	</div>\
																</div>\
	...
	$(document).on("click", "label[r-fupload-type='file.upload']", function () {
		var currentPage = ""
		if($(this).closest('.info-modal').length > 0) currentPage = "info";
		else if ($(this).closest(".register_form-wrap").length > 0) currentPage = "register";
		else if ($(this).closest(".account-info").length > 0) currentPage = "mypage";


		var REGISTER_MAX_FILE_NUMBER = 1;

    $(" div[data-jcoltype='fupload'] input[type='file']").fileupload({        
        url: '/template/register_fupload/upload-file',
        dataType: 'json',
        async: true,
        pasteZone: null,
        formData: function() {
            ...
            var current_sid = (typeof property !== 'undefined' && property.SID) ? property.SID : (typeof SID !== 'undefined' ? SID : '');
            return [
                {name: 'sid', value: current_sid}
            ];
        },			
        change : function (e, data) { // 파일 선택 후 확인 버튼 클릭할때 change 조건들 통과해야 함
            if(data.files.length > REGISTER_MAX_FILE_NUMBER || $(this).closest(".cl-s-form-control-register ").find('.file-row').length + data.files.length > REGISTER_MAX_FILE_NUMBER){
				//		    $(this).showModalFlat($.lang[LANG]['manager.list.member.block.title'], modal_content, true, true, function() {

                var modal = $(this).showModalFlat('',$.lang[LANG]['register.file.upload.limit'], modal_content, true, true, function() {
			...
			console.log("afaf");
			var maxsize = "";
			if(typeof property != 'undefined' && property) maxsize = (property.VALIDPLAN && $.inArray(property.VALIDTYPE,['BN','SM']) > -1) ? 104857600 : 10485760;
			else if(typeof VALIDTYPE != 'undefined' && VALIDTYPE) maxsize = ($.inArray(VALIDTYPE,["BUSNESS","SHOPPING"]) > -1) ? 104857600 : 10485760;
			console.log(maxsize);
            var flag = true;
            $.each(data.files, function(idx, file) {
                ...
                if(file.size > maxsize) {
					...
					$('.flat-modal').addClass('hide');
					$('.modal-backdrop').addClass('hide');
					
					var modal = $(this).showModalFlat('', err,true,false,function(){
						
						$(document).on('keydown', function(e) {
                            if(e.keyCode == 27) modal.modal('hide');
                        });
						$('.flat-modal').removeClass('hide');
						$('.modal-backdrop').removeClass('hide');

					},'ok','','cl-cmmodal cl-s-btn w560 cl-p130 cl-t80 cl-modal cl-none-title cl-close-btn',false,function(){
						$('.flat-modal').removeClass('hide');
						$('.modal-backdrop').removeClass('hide');
        ...
        add: function(e,data) { // 서버에 업로드 일단 함
            var r = $.upload_add(e,data,'register_flink');
			// r 객체에 submit이 false이고 err 메시지가 있는 경우 에러 팝업을 띄우는 처리 예시입니다.
			if (r && r.submit === false && r.err) {
				$('.flat-modal').addClass('hide');
				$('.modal-backdrop').addClass('hide');
			}
            ...
            if(r.submit) {
				...
				var modal = $(this).showModalFlat('', r.err,true,false,function(){
					$(document).on('keydown', function(e) {
                        if(e.keyCode == 27) modal.modal('hide');
                    });
					$('.flat-modal').removeClass('hide');
            ...
            if(typeof data.result.error != 'undefined' && data.result.error) {
				...
				var modal = $(this).showModalFlat('', data.result.error,true,false,function(){
            ...
            if(typeof data.result.file == 'undefined' || !data.result.file || typeof data.result.uploaded == 'undefined' || !data.result.uploaded) {
				...
				var modal = $(this).showModalFlat('', $.lang[LANG]['form.file.upload.disabled'],true,false,function(){
            ...
            if(data.result.uploaded) {
                ...
                //$(this).closest(".cl-s-etc-val").find("input[type='text']").remove();
                $(this).closest("[data-jcoltype='fupload']").find(".valueCheck").val(data.result.uploaded.orig_name + '||' + data.result.uploaded.file_size + 'KB' + '||' + data.result.file);
				...
				switch(currentPage){
					...
					case "info" :
						// 파일 정보를 조합하여 다운로드 링크 생성
						var file_info_str = uploaded.orig_name + '||' + uploaded.file_size + 'KB' + '||' + data.result.file;
						var fileResult = $.umember.fileurlcreate(file_info_str);
						var download_link = fileResult.download_link;
						...
						<div class="file-row file-info-layout">\
							...
							<div class="file-right-area">\
								<span class="file-size">'+flink_size.toFixed(2) + flink_unit+'</span>\
								'+download_link+'\
									<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">\
										<path d="M5 19v-3.5h1V18h12v-2.5h1V19H5Zm2.65-7.15.71-.71 3.15 3.15V5h1v9.29l3.15-3.15.71.71-4.35 4.35-4.35-4.35Z"/>\
									</svg>\
                ...
                $(this).closest("[data-jcoltype='fupload'] .cl-s-form-control-register").append(fileRow);
				$(this).closest("[data-jcoltype='fupload']").find('label[r-fupload-type="file.upload"]').addClass('hide');

				$(this).closest("[data-jcoltype='fupload']").addClass('isdata');
				$(this).closest("[data-jcoltype='fupload']").removeClass('nodata');
            }
            
             if($(this).closest("[data-jcoltype='fupload']").find('.file-row').length >= REGISTER_MAX_FILE_NUMBER) {
                $(this).closest("[data-jcoltype='fupload'] .cl-s-form-control-register").find('label').addClass('disabled');
                $(this).closest("[data-jcoltype='fupload'] .cl-s-form-control-register ").find('input[type="file"]').prop('disabled', true);

				var $inputElement = $(this).closest("[data-jcoltype='fupload']").find('.valueCheck');
				$inputElement.attr('data-etcidx', '');
				$inputElement.attr('data-id', '');
				$inputElement.attr('data-file-name', '');				
            }
            $.processOFF();
            // $('#loading').css('left','50%');
        },
	...
	$(document).on("click", "div[data-jcoltype='fupload'] .form-file-delete", function() {
		var REGISTER_MAX_FILE_NUMBER = 1;
		var $elem = $(this);

		var $label = $(this).closest("[data-jcoltype='fupload']").find('label');
		var $inputFile = $(this).closest("[data-jcoltype='fupload']").find('input[type="file"]');
		var etcIdx = $(this).closest("[data-jcoltype='fupload']").find('.valueCheck').attr('name');
		etcIdx = etcIdx.replace('member-', '');
		var directDeleteFlag = false;
		var $closestForm = $(this).closest('form[name="myinfo_form"]');
		var id = "";

		if( $closestForm.length > 0 && $closestForm.attr('action') === '/_myinfo_proc') {
			directDeleteFlag = true;
			id = $(this).closest($closestForm).find('[name="email"]').val();
		} else if (($(this).closest(".mng-modal-body").length > 0)){
			directDeleteFlag = true;
			id = $(this).closest(".mng-modal-body").find(".member-info-input").val();
		}

		if(directDeleteFlag) {

			var $inputElement = $elem.closest("[data-jcoltype='fupload']").find('.valueCheck');
			$inputElement.attr('data-etcidx', etcIdx);
			$inputElement.attr('data-id', id || '');
			$inputElement.attr('data-file-name', $elem.data('src'));
			
			// 파일 업로드 상태 업데이트
			var fileRowCnt = $elem.closest("[data-jcoltype='fupload']").find('.file-row').length;
			$elem.closest("[data-jcoltype='fupload']").find(".valueCheck").attr('value', '');
			
			$elem.closest("[data-jcoltype='fupload']").addClass('nodata');
			$elem.closest("[data-jcoltype='fupload']").removeClass('isdata');
			$elem.closest("[data-jcoltype='fupload']").find('label[r-fupload-type="file.upload"]').removeClass('hide');

			if($elem.closest("[data-jcoltype='fupload']").hasClass("type2")) {
				$elem.closest("[data-jcoltype='fupload']").removeClass("isdata");
				$elem.closest("[data-jcoltype='fupload']").addClass("nodata");
			}
			if(fileRowCnt - 1 < REGISTER_MAX_FILE_NUMBER) {
			...
			if(confirm(directDeleteFlag ? $.lang[LANG]['upload.file.confirm.delete'] : $.lang[LANG]['form.file.confirm.delete'])) {
			
 			$.ajax({
				url:"/template/register_fupload/delete-file",
				type:"post",
				async: true,
				beforeSend: function() {
					$.processON();
				...
				data:{
					"s": $elem.data("src"),
					"directDeleteflag" : directDeleteFlag ,
					"etcIdx" : etcIdx,
					"id" : id || "",
					"sid": (typeof property !== 'undefined' && property.SID) ? property.SID : (typeof SID !== 'undefined' ? SID : '')
				},
				dataType:"json",
				success:function() {
					var fileRowCnt = $elem.closest("[data-jcoltype='fupload']").find('.file-row').length;
					$elem.closest("[data-jcoltype='fupload']").find(".valueCheck").attr('value', '');
					if(fileRowCnt > 1) {
						$elem.closest("[data-jcoltype='fupload']").find('.file-uploaded-number').text('(' + (fileRowCnt - 1) + '/' + REGISTER_MAX_FILE_NUMBER + ')');
					} else {
						$elem.closest("[data-jcoltype='fupload']").addClass('nodata');
						$elem.closest("[data-jcoltype='fupload']").removeClass('isdata');
						$elem.closest("[data-jcoltype='fupload']").find('label[r-fupload-type="file.upload"]').removeClass('hide');
...
let startX, startY;
$('.fullcover-scroll.payment .address-item')
.live('click',function(e){
	if( $(e.target).closest('.edit btn-edit-addr').length > 0) {
		return;
	}
  ...
  .live('mouseup', function(e) {
    ...
    if (dx < 5 && dy < 5) {
      ...
      // 드래그한 것으로 판단하여 무시
      console.log('텍스트 선택 감지됨, 클릭 무시');
    }
  });

	$('.adm-member-addr').live('click',function(){
		const action = $(this).data('action');
		switch (action){
			case "adm-edit" : 
				getaddrList("manager", false, function(result) {
				});
			break;
		}
	});
	...
	// 주소 목록, 기본 주소 설정
 	$('.btn-load-addresses, .btn-default-address, .btn-add-addr, .btn-edit-addr,.address-delete').live('click',function(){
		//if (!property?.SID || !property?.UMEMBER?.id) return;
		
		const action = $(this).data('action');
		let currentPage =  '';
		const elcurPage = $('.btn-load-addresses');		
		if(elcurPage.hasClass('mypage')){
			currentPage = 'mypage';
		} else if (elcurPage.parent().hasClass('smpage-btn')){
			currentPage = 'payment';
		} else {
			currentPage = 'manager';
		}
		...
		// 기본 주소 설정하기 눌렀을 때
		if (action === 'getList') {
			getaddrList(currentPage);
		} else if (action == "add") {
			if ($('.address-item').length <= 14) {				
				editAndaddPoppup("N","add",currentPage);
			}
		} else if (action == "edit") {
			editAndaddPoppup("N","edit",currentPage,$(this));
		} else if (action =="delete") {
			deletePopup(currentPage,$(this));
			...
			$.ajax({
				type: 'POST',
				url: '/umember/get/type/smAddressList',
				data: {
				sid: property.SID,
				uid: property.UMEMBER.id,
				seq: seq,
				action: 'setDefault'
				},
				dataType: 'json',
				success: function (data) {
					...
					if (data?.error) {
						...
						//$addresslist_popup.one('hidden.bs.modal', function () {
							// setTimeout(function() {
							// 	getaddrList(currentPage);
							// 	updateAddressUI(data,currentPage);
														
							// }, '50');
								getaddrList(currentPage,true);

								if(currentPage == "payment"){
									var elSeq = $('.sm-parent').attr('data-smseq');
									var elDefault = $('.sm-parent').find('.sm-default-addr').hasClass("hide");

									if(!elDefault || elSeq == seq){
										updateAddressUI(data,currentPage);
									}
								} else {
									updateAddressUI(data,currentPage);
	...
	window.checkNumberPopup = function(el) {
		var val = el.val();
		el.val(val.replace(/[^0-9]/g,''));
	}
	// 주소 가져오기 
	var getaddrList = function(currentPage,changeHtml = false ) {
 		$.ajax({
			type: 'POST',
			url: '/umember/get/type/smAddressList',
			data: {
			sid: (typeof property !== 'undefined' && property?.SID) ? property.SID : SID,
			uid: (currentPage=='manager') ? $('.memberinfo-modal .member-info-input').val() :  property.UMEMBER.id,
			action: 'getList'
			},
			dataType: 'json',
			success: function (data) {
				// if(currentPage === "manager" && callback){
				// 	callback(data);
				// 	return true;
				// } 
				if (data?.error) {
				...
				if (data.length > 0) {
					...
					var changehtm = showList(data, currentPage,changeHtml);
					if(changehtm){
						console.log(changehtm);
						const $tempDom = $('<div>').html(changehtm); 
						const $newItems = $tempDom.find('li.address-item');

						const $list = $addresslist_popup.find('ul.address-list');
					...
					if(data.length >= 15) {
						...
						$('.btn-add-addr').addClass('pointer-none');
					}					
					
				} else {	
					// 가져온 주소x 
					editAndaddPoppup("Y","add",currentPage);
				}
	...
	// 주소 그리기
	var showList = function(d,currentPage,changeHtml = false) { 
		
		if(currentPage == "manager") {
			$('.memberinfo-modal').closest('.flat-modal').hide();
			$('.memberinfo-modal').closest('.flat-modal').next('.modal-backdrop').hide();
		}
		var content_li = '';
		$.each(d,function(i,v){
			const isDefault = v.is_default == 1 ? 'show' : 'hide';
			const name = v.to_name;
			const alias = v.alias || '';
			const aliasel = (v.alias) ? 'open' : 'hide';
			const seq = v.seq;			
			const tel = v.to_tel;
			
			
			const to_addr1 = v.to_addr1 || '';
			let postCode = '';
			let addr1 = '';

			if (to_addr1.includes('||')) {
			const parts = to_addr1.split('||');
			...
			const addr2 = v.to_addr2 || '';
			content_li += `
			<li class="address-item">
				<div class="div1">
					<span class="name">${name}</span> 
					<span class="tag ${!alias ? 'hide' : ''}" edit-tag="${escape(alias)}">${alias ? '(' + alias + ')' : ''}</span> 
					<span class="default ${isDefault}">기본 배송지</span>
					<div class="newcheckbox hand">
						<div class="newcheckboxSvgRadio">
						<label>
							<input type="radio" name="temp" value="${seq}"  ${v.is_default == 1 ? 'checked' : ''}>
							<svg viewBox="0 0 18 18" width="18" height="18">
								<path fill="#dbdcdf" d="M9 0C4.03 0 0 4.03 0 9c0 4.97 4.03 9 9 9s9-4.03 9-9C18 4.03 13.97 0 9 0zM9 17c-4.41 0-8-3.59-8-8 0-4.41 3.59-8 8-8s8 3.59 8 8C17 13.41 13.41 17 9 17z"></path>
							</svg>
							<svg class="active" viewBox="0 0 18 18" width="18" height="18">
								<path fill="#4789e7" d="M9 0C4.03 0 0 4.03 0 9c0 4.97 4.03 9 9 9s9-4.03 9-9C18 4.03 13.97 0 9 0zM9 17c-4.41 0-8-3.59-8-8 0-4.41 3.59-8 8-8s8 3.59 8 8C17 13.41 13.41 17 9 17z"></path>
								<circle fill="#4789e7" cx="9" cy="9" r="6"></circle>
							</svg>
						</laberl>		
						</div>
					</div>
				...
				<div  class="div2">
					<span class="address1 ${!postCode ? 'hide' : ''}" edit-postcode="${postCode}">${postCode ? '[' + postCode + ']' : ''}</span><span class="address2" edit-addr1="${escape(addr1)}" edit-addr2="${escape(addr2)}">${addr1} ${addr2}</span>
				</div>
				<div class="div3 phone">${tel}</div>
				<div class="div4 actions clearfix">
					<button class="edit btn-edit-addr" data-action="edit" ><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12 12" class="address-edit" data-toggle="tooltip" data-placement="top" title="" data-original-title="수정" width="12" height="12"><path d="m11.56.57-.13-.13C11.14.15 10.76 0 10.37 0c-.38 0-.77.15-1.06.44L.75 9 0 12l3-.75 8.56-8.56c.59-.59.59-1.54 0-2.12zm-9.07 9.78-1.11.28.28-1.11 6.58-6.58.84.84-6.59 6.57zm8.36-8.37L9.77 3.06l-.83-.83 1.08-1.08a.487.487 0 0 1 .7 0l.13.13c.2.19.2.51 0 .7z"></path></svg></button><button class=" ${currentPage == "manager" ? 'hide' : ''} delete ${(v.is_default == 1) ? 'hide' : ''}"><svg viewBox="0 0 12 13" width="12" height="13" class="address-delete" data-toggle="tooltip" data-placement="top" title="" data-original-title="삭제" data-action="delete"><path d="M11.01 2H9V0H3v2H0.99 0v1h1v8c0 1.1 0.9 2 2 2h6c1.1 0 2-0.9 2-2V3h1V2H11.01zM4 1h4v1H4V1zM10 11c0 0.55-0.45 1-1 1H3c-0.55 0-1-0.45-1-1V3h1.02H9h1V11z"></path><rect x="4" y="4" width="1" height="7"></rect><rect x="7" y="4" width="1" height="7"></rect></svg></button>
					<div data-action="setDefault" seq ="${v.seq}" class="  ${currentPage == "manager" ? 'hide' : ''} btn-default-address ${(v.is_default == 1) ? 'hide': ''}" style="">기본 배송지 설정</div>
				</div>
			</li>
		`;
		});
		...
		var modal_content =`
		<div class="btn-add-addr  ${currentPage == "manager" ? 'hide' : ''}"  data-action="add" >
			<div>
		...
		<ul class="fullcover-scroll address-list ${currentPage}">
			${content_li}
		</ul>
		`		 
		if(changeHtml) return content_li;
		$addresslist_popup = $(this).showModalFlat("배송지 목록", modal_content, true, (currentPage == "mypage" ||currentPage == "manager" ) ? false : true , function(){
			
			
				// 결제 페이지의 목록 팝업에서 확인 버튼 클릭 시
				$("#no-fouc").removeClass("scroll-lock-fullcover-popup");
				var seq = $addresslist_popup.find('.address-item input[type="radio"]:checked').attr('value');
		...
		},   (currentPage == "payment" ? 'cancel' : 'close' )  , 'config.save', 'cl-cmmodal cl-s-btn w560 cl-p70 cl-p0  addr-list-parents cover fullcover-modal fullcover-modal-scroll  ' + ((currentPage === 'manager') ? "manager" : "") + '   ',true,function(){
			$("#no-fouc").removeClass("scroll-lock-fullcover-popup");

			if(currentPage == "manager") {
				$('.memberinfo-modal').closest('.flat-modal').show();
				$('.memberinfo-modal').closest('.flat-modal').next('.modal-backdrop').show();
			}
		},function(){

			// if($addresslist_popup.closest('#flat-modal1').length >0 ){
			// 	$addresslist_popup.closest('#flat-modal1').attr('id','flat-modal');
			// }

		});
		
		if($("#no-fouc").addClass("scroll-lock-fullcover-popup"));
	  
		
	}
	
	// 마이페이지 팝업에서 주소를 등록하거나 결제 페이지 팝업에서 저장 버튼을 클릭했을 때 데이터 반영
	var updateAddressUI = function(d,cPage){
			
		if(cPage == "mypage"){
			var removeEl = $('.sm-parent.emptyFlag'); // 요소 자체를 담음
			if (removeEl.length) { 
				// optional 클래스 제거
				removeEl.find('.cl-s-form-wrap.empty').removeClass('empty');
				removeEl.find('.optional').removeClass('optional');
				// flexbox 안의 input 제거
				removeEl.find('.flexbox').children('input').remove();
			}
		} 
			

		var $sm = $('.sm-parent');
		var $sm_btn = $sm.find('.btn-load-addresses div span');
		var $sm_name = $sm.find('.sm-name');
		var $sm_alias = $sm.find('.sm-alias');
		var $sm_default = $sm.find('.sm-default-addr');

		const $sm_addr1 = $sm.find('.sm-addr1 span').first();   // [우편번호]
		const $sm_addr2 = $sm.find('.sm-addr1 span').last();    // 주소
		const $sm_tel = $sm.find('.sm-tel');  				    // 주소2

		d?.alias ? $sm_alias.text('(' + d.alias + ')').removeClass('hide') : $sm_alias.text('').addClass('hide');
		
		$sm.removeClass('emptyFlag');
		

		
		(cPage == "mypage") ? $sm_btn.text('변경하기') : $sm_btn.text('배송지 목록');
		
		$sm_name.text(d?.to_name);
		
		const tempArr = d.to_addr1?.split('||') || [];			
		tempArr[0] ? $sm_addr1.text('[' + tempArr[0] + ']').removeClass('hide') : $sm_addr1.addClass('hide');
		$sm_addr2.text(tempArr[1] +' '+d?.to_addr2);
		$sm_tel.text(d?.to_tel);
		$sm.find('.noaddress').addClass('hide');

		if($('.btn-load-addresses.empty').length > 0) {
			$('.btn-load-addresses.empty').removeClass('empty');
		}

		if(cPage == "payment") {

			if (+d.is_default > 0) {
			$sm_default.removeClass("hide");
			} else {
			$sm_default.addClass("hide");
			}				
			
			$('#reciever-name').val(d.to_name);
			$('#reciever-phone').val(d.to_tel);
			$('#reciever-postcode').val(tempArr[0]);
			$('#reciever-addr1').val(tempArr[1]);
			$('#reciever-addr2').val(d.to_addr2);
			changeDeliveryPrice($(this).closest('form').attr('data-sid'));
			console.log($(this));
		}
	}

	var editAndaddPoppup = function(fix_addr = "N",  mode = "", cpage = "",select = ""){
		var dontclick_label = "";		
		var opa = "";

		if (fix_addr === "Y") {
			dontclick_label = 'disabled checked';
			opa = "opa";
		} else if (mode === "edit") {
			var $a_item = select.closest('.address-item');

			if ($a_item.find('.default.show').length > 0) {
				dontclick_label = 'disabled checked';				
				opa = "opa";
			}
		}
		...
		var modal_content =  `
					<div>
						<div class="cl-s-info">배송지 정보</div>
						<div class="col-xs-12 col-sm-12 col-md-12 no-padding">
							<div class="cl-s-form-wrap">
								<div class="cl-s-form-group">
									<input type="text" class="cl-s-form-control-popup" id="addrPopup-alias"name ="alias" required>
									<label for="input" class="cl-s-control-label">배송지명</label>
								</div>
							</div>
						</div> 					
						<div class="col-xs-6 col-sm-6 col-md-6 no-padding p-r-5">
							<div class="cl-s-form-wrap">
								<div class="cl-s-form-group">
									<input type="text" class="cl-s-form-control-popup" id="addrPopup-name" name="to_name" required="required">
									<label for="input" class="cl-s-control-label">이름<span class="input-star">*</span></label>
									
								</div>
							</div>
						</div>
						<div class="col-xs-6 col-sm-6 col-md-6 no-padding p-l-5">
							<div class="cl-s-form-wrap">
								<div class="cl-s-form-group">
									<input type="text" class="cl-s-form-control-popup" id="addrPopup-phone" name="to_tel" required="required" onkeyup="checkNumberPopup($(this))">
									<label for="input" class="cl-s-control-label">연락처<span class="input-star">*</span></label>
								</div>
							</div>
						</div>
						<div class="col-xs-12 col-sm-12 col-md-12 no-padding clearfix">
							<div class="cl-s-form-wrap">
								<div class="cl-s-form-group">
									<input type="hidden" class="cl-s-form-control-popup" id="addrPopup-postcode" name="addr1" required="required" value="">
									<input type="text" class="cl-s-form-control-popup" id="addrPopup-addr1" name="addr2" required="required" readonly onfocus="addrSearchModal('addrPopup')">
									<label for="input" class="cl-s-control-label">주소<span class="input-star">*</span></label>
								</div>
							</div>
							<div class="cl-s-form-wrap">
								<div class="cl-s-form-group">
									<input type="text" class="cl-s-form-control-popup" id="addrPopup-addr2" name="addr3" required>
									<label for="input" class="cl-s-control-label">상세주소</label>
								</div>
							</div>
						</div> 
						<span class="pull-left checkbox-wrap ${cpage == "manager" ? 'hide' : ''}">
							<div class="cl-s-checkbox ${opa}">
								<label>
									<input type="checkbox" id="addrPopup-set" name="is_default" ${dontclick_label}>
									<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16"><path d="M13 0H3C1.3 0 0 1.3 0 3v10c0 1.7 1.3 3 3 3h10c1.7 0 3-1.3 3-3V3C16 1.3 14.7 0 13 0zM15 13c0 1.1-0.9 2-2 2H3c-1.1 0-2-0.9-2-2V3c0-1.1 0.9-2 2-2h10c1.1 0 2 0.9 2 2V13z"></path></svg>
		...
		var $addPopup = $(this).showModalFlat( (mode == "add") ? "배송지 추가" : "배송지 수정" , modal_content, true, true , function(){
			
			$addPopup.find('.cl-s-form-wrap').removeClass('empty');

			let focused = false;
			let flagCheck = false;

			$addPopup.find('.cl-s-form-wrap').each(function (i,e) {
				const $wrap = $(this);
				const $inputs = $wrap.find('input:not([type=hidden]), textarea');
				const hasError = $wrap.hasClass('error');

				let isEmpty = false;

				// 오직 required="required" 로 명시된 필드만 검사
				$inputs.each(function () {
					...
					if (input.getAttribute('required') === 'required') {
						...
						const $wrap = $(input).closest('.cl-s-form-wrap');
						const $errorLabel = $wrap.next('.cl-s-control-label.error');
						...
						if (!val) {
							...
							switch($input_id){
								case "addrPopup-name" :
									error_str = " 이름을 입력해 주세요.";
									break;
								case "addrPopup-phone" :
									error_str = " 연락처를 입력해 주세요."
									break;
								case "addrPopup-addr1" :
									error_str = " 주소 찾기를 통해 주소를 입력해주세요."									
									break;
							}
							// 새 에러 추가
							$wrap.after(
								'<label for="input" class="cl-s-control-label error"><i class="cl-icon alert02"></i>' + error_str + '</label>'
							);
						}
					}
				});
				...
				// 포커스: 첫 번째 error 또는 empty 항목
				if ((isEmpty || hasError) && !focused) {
					flagCheck = true;
					const $focusTarget = $wrap.find('input, textarea').filter(':visible').first();
			...
			const formValues = {
				to_name: $addPopup.find('#addrPopup-name').val()?.trim(),
				to_tel: $addPopup.find('#addrPopup-phone').val()?.trim(),
				to_addr1: `${$addPopup.find('#addrPopup-postcode').val()?.trim()}||${$addPopup.find('#addrPopup-addr1').val()?.trim()}`,
				to_addr2: $addPopup.find('#addrPopup-addr2').val()?.trim(),
				alias: $addPopup.find('#addrPopup-alias').val()?.trim(),				
				is_default: $addPopup.find('#addrPopup-set').prop('checked') ? '1' : '0'
			};
			...
			if(mode == "add"){
				$.ajax({
					...
					success: function (data) {
						if (data?.error) {
							...
							if( typeof $addresslist_popup !== "undefined" && $addresslist_popup.length > 0) {
								$addresslist_popup.modal('hide');
							}

							if(cpage == "mypage" && formValues.is_default == "1"){
								updateAddressUI(formValues,cpage);
							} 
							else if (
								(cpage == "payment" && fix_addr == "Y") ||
								(cpage == "payment" && formValues.is_default > 0 && $('.sm-default-addr:not(.hide)').length > 0)
							) {
								updateAddressUI(formValues,cpage);
								UpdateelSeq(data["seq"]);
							}
							$addPopup.modal('hide');							
							getaddrList(cpage);
						}
				...
				var $tempitem = select.closest('.address-item');
				var radio_value = $tempitem.find('input[type="radio"]').val() || '';
				if (!radio_value) return true;
				
				if(cpage == "manager" && $('#member-postcode').length > 0 && formValues.is_default > 0) {
					$('#member-addr1').val($addPopup.find('#addrPopup-addr1').val()?.trim()) ;
					$('#member-addr2').val(formValues.to_addr2) ;
					console.log('afaf');
					$('#member-postcode').attr('value',$addPopup.find('#addrPopup-postcode').val()?.trim());	
					
					var temp_uid = $('.memberinfo-modal .member-info-input').val();
					$('.mng-list table tr[data-uid="' + temp_uid + '"]').find('.'+'addr').text(formValues.to_addr1 + formValues.to_addr2);
					...
					$.ajax({
						...
						data: {
						sid: (typeof property !== 'undefined' && property.SID) ? property.SID : SID,
						uid: (cpage=='manager') ? $('.memberinfo-modal .member-info-input').val() :  property.UMEMBER.id,
						action: 'edit',
						seq : radio_value,
						value : formValues
						},
						...
						success: function (data) {
							if (data?.error) {
								...
								if(cpage =="payment" && !elDefault &&  formValues["is_default"] == 1 || 
								   cpage =="payment" &&  elSeq == radio_value
								){
									updateAddressUI(formValues,cpage);
								} else if(cpage != "payment"){
		...
		}, 'cancel', (cpage === 'manager' ? '저장' : 'config.save'), 'cl-cmmodal cl-s-btn w560 cl-p70 cl-p0 parallax-description-modal cl-s-add-addrPopup cover fullcover-modal ' + ( (cpage == "manager") ? 'manageredit' : '')  +'',true,function(){
			console.log('a');
		},function(){});		

		if(mode == "edit") {
			if($addresslist_popup.length > 0) {

				var $item = select.closest('.address-item');

				var name_edit = $item.find('.name').text().trim();
				var aliasAttr = unescape($item.find('.tag').attr('edit-tag'));
				var alias_edit = aliasAttr ? aliasAttr.trim() : '';
				var is_default_edit = $item.find('.default.show').length > 0 ? '1' : '0';
				var postcodeAttr = $item.find('[edit-postcode]').attr('edit-postcode');
				var postcode_edit = postcodeAttr ? postcodeAttr.trim() : '';
				var addr1Attr = unescape($item.find('[edit-addr1]').attr('edit-addr1'));
				var address1_edit = addr1Attr ? addr1Attr.trim() : '';
				var addr2Attr = unescape($item.find('[edit-addr2]').attr('edit-addr2'));
				var address2_edit = addr2Attr ? addr2Attr.trim() : '';
				var phone_edit = $item.find('.div3.phone').text().trim();
				if (mode === 'edit') {
						$addPopup.find('#addrPopup-alias').val(alias_edit);
						$addPopup.find('#addrPopup-name').val(name_edit);
						$addPopup.find('#addrPopup-phone').val(phone_edit);
						$addPopup.find('#addrPopup-postcode').val(postcode_edit);
						$addPopup.find('#addrPopup-addr1').val(address1_edit).addClass('has-value');
						$addPopup.find('#addrPopup-addr2').val(address2_edit);
						$addPopup.find('#addrPopup-set').prop('checked', is_default_edit === '1');
				}
			}
		}	 
	}	

	var deletePopup = function(cpage,c_this){

		$addresslist_popup.addClass('hide');
		var $tempitem = $(c_this).closest('.address-item');
		var radio_value = $tempitem.find('input[type="radio"]').val() || '';
						
		var $addr_modal_delete = $(this).showModalFlat("주소를 삭제하시겠습니까?", "삭제한 주소는 복구할 수 없습니다. <br> 정말 삭제하시겠습니까?", true, true , function(){
			...
			$.ajax({
				...
				success: function (data) {		
					...
					$addr_modal_delete.modal('hide');
					$addresslist_popup.modal('hide'); 
				}
			});

		}, 'cancel','ok','modal-dialog cl-cmmodal cl-s-btn w560 cl-p70',false,'','',function(){
			$addresslist_popup.removeClass('hide');
		});
				

	}

	var UpdateelSeq = function(seq){
		$('.sm-parent').attr("data-smseq",seq);
	}
$('.btn-add-addr.pointer-none').live('click',function(e) {

	

   var modal = $(this).showModalFlat($.lang[LANG]['config.information'], $.lang[LANG]['shopping.addr.title.dec'], true, false, function(){
	
	$backModal = $('#flat-modal').closest('.cl-common-modal').css('opacity',1)		
	if ($backModal.next().hasClass('modal-backdrop')) {
		$backModal.next().css('opacity', 0.9);  // 원하는 스타일
	}
	}, 'ok', '', 'cl-p130 cl-cmmodal cl-s-btn w560 cl-p0 cl-okbtn-pbt70',true,function(){
	$backModal = $('#flat-modal').closest('.cl-common-modal').css('opacity',1)		
	if ($backModal.next().hasClass('modal-backdrop')) {
		$backModal.next().css('opacity', 0.9);  // 원하는 스타일
	}
	...
	$backModal = $('#flat-modal').closest('.cl-common-modal').css('opacity',0)		
	if ($backModal.next().hasClass('modal-backdrop')) {
		$backModal.next().css('opacity', 0);  // 원하는 스타일
	}
	...
	$('.cl-s-add-addrPopup .cl-s-form-group').live('click',function(e) {
		if($(e.target).hasClass('cl-s-form-group')) {
			$(e.target).find('.cl-s-form-control-popup:not([type=hidden])').focus();
		}
	});
	$('.cl-s-form-control-popup').live({
		...
		focus: function(e) {
			...
			if($(this).is('#addrPopup-addr2') && $('#addrPopup-addr1').val().trim().length > 0 ) {

				const formWrap = $('#addrPopup-addr1').closest('.cl-s-form-wrap');
				const nextElement = formWrap.next('.cl-s-control-label');

				if (nextElement.length > 0) {
					nextElement.remove();
				}
				var input_id = $(this).attr('id'),
					input_addr1 = input_id.replace('-addr2','-addr1');
					console.log(input_addr1);
				if($('#'+input_addr1).val().trim().length > 0) {
					$('#'+input_addr1).closest('.cl-s-form-wrap').removeClass('empty');
				}
			}
		...
		blur: function(e) {
			...
			if($(this).attr('type') == 'hidden' || $(this).is('textarea')) {
				e.preventDefault();
				return false;
			}
			...
			if (val.length === 0 && this.getAttribute('required') === 'required') {
				...
				$(this).closest('.cl-s-form-wrap').removeClass('empty');
				$(this).closest('.cl-s-form-wrap').next('.cl-s-control-label.error').remove();

				if(val.length > 0 && checkEmojis(val)){
				 	error_str = $.lang[LANG]['config.unable.emoji'];
					
				} else if( ($(this).is('#addrPopup-alias') || $(this).is('#addrPopup-name')) &&  val.length > 50) {
					      error_str = '50자 이내로 입력해 주세요.';
				}
			...
			$.each(data, function(k,o) {
				...
				switch(k) {
					...
					case 'addr1':
						...
						var sm_addr =  (data && data.umseq) ? data.umseq : '',
							sm_name =  (sm_addr && sm_addr.to_name) ? sm_addr.to_name : '',							
							sm_alias = (sm_addr && sm_addr.alias) ? sm_addr.alias : '',
							sm_tel = (sm_addr && sm_addr.to_tel) ? sm_addr.to_tel : '',
							addr_list_btn ='';
							addr_default_html = '';
							if(sm_addr || checkSM) {
								
								addr_list_btn += '\
								<div class="cl-s-form-wrap btn-load-addresses adm-member-addr '+(!sm_addr? 'hide' : '' )+' " data-action="adm-edit">\
									<div class="cl-s-form-group "><span>배송지 목록</span>\
									</div>\
								</div>';		
								
								addr_default_html += '\
								<div class="sm-parent '+ (!sm_addr ? 'emptyFlag' : '') +'">\
									<div class="cl-s-form-wrap">\
										<div class="cl-s-form-group">\
											<div class="cl-s-form-control"><div class="sm-name">'+sm_name+'</div><div class="sm-alias ' +(sm_alias ? '':'hide')+ '">' +(sm_alias ? '('+sm_alias+')' : '' )+ ' </div><div class="sm-default-addr"><span>기본배송지</span></div></div>\
											<div class="cl-s-form-control"><div class="sm-addr1"><span class=" '+ (addr[0]?.trim() ? '' : 'hide')+' "> '+(addr[0]?.trim() ? '['+ addr[0]?.trim() +']' : '' )+'</span><span>'+  addr[1].trim() +' '+ addr[2].trim() +'</span></div></div>\
											<div class="cl-s-form-control"><div class="sm-tel">'+sm_tel+'</div></div>\
											<div class="cl-s-form-control noaddress" style="text-align:center; padding:20px; color:#999;">등록된 배송지가 없습니다.</div>\
										</div>\
									</div>\
						...
						if(list_type == 'member-list') {
							if(sm_addr || checkSM) { list_str += addr_list_btn; }
							...
							list_str += '\
								...
								<div class="cl-common-form-wrap nonplace-holder">\
			   					...
			   					<div class="cl-common-form-wrap nonplace-holder">\
			   						<div class="cl-common-form-group">\
                                       	<input class="member-info-input form-control" id="member-addr2" type="text" value="' + escapeHtml(addr[2].trim()) + '" name="member-addr2" required="required">\
			 						</div>\
			   					</div>\
							';

							if (sm_addr || checkSM) { list_str += addr_default_html;}							
						} else {
							list_str += '\
								<input class="hide" id="member-postcode" type="hidden" value="' + addr[0].trim() + '">\
					...
					default:
						...
						if(list_type == 'member-list') {
							...
							switch (cl_type) {
								...
								case 'fupload':
									// 단일 파일 업로드 타입일 경우 다운로드 링크 생성
									// view.html과 동일하게 explode 사용

									// fileurlcreate 함수를 사용하여 파일 URL 생성
									var fileResult = $.umember.fileurlcreate(cl_val);
									var download_link = fileResult.download_link;
									var file_data = fileResult.file_data;
									cl_type_str += $.umember.fileHtmlRender(k, {
										//cpage : (check_page =='mypage') ? 'mypage' :'join',
										val : file_data || '',
										type : "type2",
										type2_downlink : download_link,
									})	
							...
							if(cl_type=='fupload' && cl_val) {
								var templink = $.umember.fileurlcreate(cl_val|| '');

								var tempdownicon = '<svg class="file-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24"><path d="M5 19v-3.5h1V18h12v-2.5h1V19H5Zm2.65-7.15.71-.71 3.15 3.15V5h1v9.29l3.15-3.15.71.71-4.35 4.35-4.35-4.35Z"/></svg>'
								cl_val = ''
								+ '<div class="fupload-cell">'
								+   '<span class="file-name">' + templink.file_info.name + '</span>'
								+     templink.download_link  // <a href="... " class="file-download-btn" target="_blank">
								+       tempdownicon          // <svg ...>
								+     '</a>'
								+ '</div>';								
							}
				...
				var info_modal = $(this).showModalFlat($.lang[LANG]['manager.member.details.title'], modal_content, true, !checkViewMode, function() {
					...
					$('.info-modal').find('.form-control:not([disabled])').each(function(i) {
						...
						if(!check_error) {
							...
							// deletes 클래스가 있는 파일들 수집
							var deletedFiles = [];
							$('.info-modal .valueCheck[data-etcidx]').each(function() {
								var $inputElement = $(this);
								
								deletedFiles.push({
									file_name: $inputElement.attr('data-file-name'),
									etcIdx: $inputElement.attr('data-etcidx'),
									id: $inputElement.attr('data-id')
								});
							});
							$.processON();
							...
							$.ajax({
								...
								data: { sid : SID, val: JSON.stringify(info_data), uid : uid ,addrnoupdate : data.umseq,deleted_files: JSON.stringify(deletedFiles)},
								dataType: 'json',
								async: true,
								success: function(data) {
									...
									$.processOFF(); // 성공 시에도 processOFF 호출
									...
									$.each(info_data, function(k, v) {
										...
										// fupload 타입 값인지 확인하고 기존 함수 활용
										if (typeof v === 'string' && v.indexOf('storage.googleapis.com/cr-resource/flink') > -1) {
											var fileResult = $.umember.fileurlcreate(v);
											if (fileResult.download_link) {
												v = fileResult.download_link + 
													fileResult.file_info.name + ' (' + fileResult.file_info.size_display + ')</a>';
											}
										}
										// HTML이 포함된 경우 html() 메서드 사용
										if (typeof v === 'string' && v.indexOf('<a') > -1) {
											$('.mng-list table tr[data-uid="' + uid + '"]').find('.'+k).html(v);
										} else {
								...
								error: function(xhr, status, error) {
									...
									$.processOFF(); // 에러 시에도 processOFF 호출
								}
							});
						}
...
var collectionItem = function(name,etc) {
	...
	var jcollection_select_type = ['text','memo','radio','check','select','fupload'],
