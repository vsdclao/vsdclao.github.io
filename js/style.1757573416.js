var style = new function () {
	...
	this.get = function(j,selector) {
		...
		if(typeof j == 'undefined') return base.property; 
	...
	var getMenuCssSelector = function(key, selector, version) {
		if($.inArray(version, ['moption','clnav']) > -1) return {'delete': {}, 'set': {}};
	...
	this.setCssProperty = function(j,selector,key,val) {
		...
		if(selectEL == 'el-menu') {	
			...
			if($('.'+selectEL).find('.cl-nav').length > 0) version = 'clnav';
	...
	this.deleteRootCssProperty = function(j,selector,key) {
		if(typeof key == 'undefined') key = [];
		else if(typeof key == 'string') key = key.split(',');
		...
		if(key.length == 0) {
			...
			$.each(key, function(i,k) {
				var checkRootkeyAll = (/-$/.test(k)) ? true : false,
					regex = new RegExp('^' + k);

				if(typeof j['children'][selector]['attributes'] != 'undefined') {
					if(checkRootkeyAll) {
						$.each(j['children'][selector]['attributes'], function(root_key,root_val) {
							if(regex.test(root_key)) delete j['children'][selector]['attributes'][root_key];
						});
					} else if(typeof j['children'][selector]['attributes'][k] != 'undefined') {
						delete j['children'][selector]['attributes'][k];
					}
				}
			});
	...
	this.selectTabsCategoryColorSetForm = function(cls,fn,el,elname,colortype,tab_case) { 
		var now_palette_html = '',
			now_palette_arr = {},
			pn = '',
			selector = {
	...
	this.selectTabsNaviColorSetForm = function(cls,fn,el,elname,colortype,tab_case) { 
		var now_palette_html = '',
			clr_picker = '',
			pn = 'fill',
	...
	this.selectCalendarLine = function(cls,fn,elname) { 
		var now_palette_html = '',
			clr_picker = '',
			pn = '--fc-border-color',
			selector = '.'+elname+' .fc',
		...
		clr_picker = getColorisHtml(color,'calendarconfig',{'pn':pn, 'el':selectEL, 'selector':selector},'thumbnail',true,{'input':'clr-alphaoff clr-tooltipoff'});
		now_palette_html += `
				<div class="calendar-line-color">
					${clr_picker}
				</div>
		`;
	...
	this.selectModooCalendarSetForm = function(fn, elname) { 
		var now_palette_html = '',
			clr_picker = '',
			pn = '--site-theme-color',
			selector = '.'+elname,
		...
		clr_picker = getColorisHtml(color,'modooCalendar',{'pn':pn, 'el':selectEL, 'selector':selector},'thumbnail',true,{'input':'clr-alphaoff clr-tooltipoff'});
		now_palette_html += `
				<div class="arrow-color">
					${clr_picker}
				</div>
	...
	this.selectTabAlignForm = function(cls,fn,elname) {
		...
		str = '\
			<div class="btn-group btn-group-sm" data-toggle="buttons" role="group" aria-label="gallery category align">\
				<label class="ctrl-tab-align-el '+LANG+'" data-tab-align="flex-start" data-elname="'+elname+'"><input type="radio" name="gcalign" value="left" id="gcAlignLeft" '+ta_l+' />'+ $.lang[LANG]['config.label.align.left'] +'</label>\
				<label class="ctrl-tab-align-el '+LANG+'" data-tab-align="center" data-elname="'+elname+'"><input type="radio" name="gcalign" value="center" id="gcAlignCenter" '+ta_c+' />'+ $.lang[LANG]['config.label.align.center'] +'</label>\
				<label class="ctrl-tab-align-el '+LANG+'" data-tab-align="flex-end" data-elname="'+elname+'"><input type="radio" name="gcalign" value="right" id="gcAlignRight" '+ta_r+' />'+ $.lang[LANG]['config.label.align.right'] +'</label>\
			</div>\
		';
		return str;
	}
	...
	this.menuColorSetForm = function(j,selector) {
		...
		var str = '\
			<div class="elmenu-color-sidebar bc-style">\
	...
	this.checkBlockRootCSS = function(tp) {
		...
		switch(tp) {
			...
			case 'sync-dataview':
				check_properties.push('--sync-page-unit');
				break;
			...
			case 'rollover-color':
				check_properties.push('--rollover-color','--rollover-bgcolor','--rollover-line');
				break;
    ...
    this.cssNumber = function(val) {
        return parseFloat(String(val).replace(/[^0-9.-]/g, '').replace(/(\..*?)\..*/g, '$1'));
    }
    this.onlyNumber = function(val) {
