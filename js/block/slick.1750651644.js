            ...
            success: function(r) {
                ...
                if(sync_list_total > 0) {
                    ...
                    $.each(r.data.list, function(r_i,r_v) {
                        ...
                        if(r_v.sale_rate > 0) $tmpItem.find('.sale').text(r_v.sale_rate + '%');
                        ...
                        if(r_v.product_unit_str) {
                            var tmp_unit_selector = ($tmpItem.find('ul.price-wrap').length > 0) ? `ul.price-wrap` : `.content`,
                                tmp_unit_wrap = ($tmpItem.find('ul.price-wrap').length > 0) ? `li` : `div`;
                            $tmpItem.find(tmp_unit_selector).append(`<${tmp_unit_wrap} class="price-unit-pricing">${r_v.product_unit_str}</${tmp_unit_wrap}>`); // 단위당 가격
                        }
                    ...
                    if(checkSyncGalleryFrame && typeof sync_gallery.type != 'undefined') {
                        appendGalleryFrame($sync, sync_gallery.pid, sync_gallery.type, sync_gallery.elsettings);
                    }
                } else {
    ...
    $(document)
        .on('click', '[data-type="sync"] .sync-page-more', function(e) {
            ...
            var suncLoadMore = $(this),
                ...
                sync_display_cnt = ($('.'+sync_selector).length > 0) ? Number(getComputedStyle(document.querySelector('.'+sync_selector)).getPropertyValue('--display-count')) || 10 : 10;
                sync_display_page = ($('.'+sync_selector).length > 0) ? Number(getComputedStyle(document.querySelector('.'+sync_selector)).getPropertyValue('--display-page')) || 1 : 1;

            sync_display_page++;
			document.querySelector('.'+sync_selector).style.setProperty('--display-page', sync_display_page, null);
            $(this).closest('[data-type="sync"]').find(`.sync-item:gt(${(sync_display_cnt*sync_display_page)+1})`).addClass('hide');
            $(this).closest('[data-type="sync"]').find(`.sync-item:lt(${sync_display_cnt*sync_display_page})`).removeClass('hide');

            var sunc_p_total = (suncLoadMore.find('.sync-page-label').length > 0) ? Number(suncLoadMore.find('.sync-page-label .sync-page-total').text()) || 1 : 1;
            if(sunc_p_total == sync_display_page) suncLoadMore.closest('.sync-page-wrap').hide();
            else suncLoadMore.find('.sync-page-label .sync-page-now').text(sync_display_page);
        })
